...
function addTaskToCalendar(task) {
    ...
    $('#calendarTaskView').fullCalendar('renderEvent', event);
}


$(document).ready(function() {    
    $('#calendarTaskView').fullCalendar({
        header: {
        ...
        events: function(start, end, timezone, callback) {
            $.ajax({
                url: BASE_URL + 'backend/edittask_management.php?action=fetch_calendar_events_single',
                type: 'GET',
                data: {
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        const events = response.data.map(item => ({
                            ...
                            start: item.task.task_date + 'T' + item.task.start_time,
                            end: item.task.task_date + 'T' + item.task.end_time,
                            allDay: false
                        }));
                        callback(events);
                    }
